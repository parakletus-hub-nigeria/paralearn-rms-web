...
const ResetPassword = ({ params }: props) => {
    ...
    const formData = [
        ...
        },
        {
            ...
            type: "password",
            subtext: "Passwords do not match"
        }
    ]
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name == 'password') {
            value.length >= 8 ? setPassword(true) : setPassword(false)
        }
        if (name == 'confirmPassword') {
            value === data.password ? setConfirmPassword(true) : setConfirmPassword(false)
        }
    }
            ...
            <div className="w-[95%] md:w-[70%] lg:w-[45%] flex flex-col items-center justify-between py-[40px] min-h-[445px] h-auto mt-[30px] mb-[30px]">
                {exp ? (
                    <div className="w-[90%] space-y-5 flex flex-col items-center">
                        ...
                        <p className="text-center text-sm">This reset link has already expired. Please request a new one.</p>
                        <Link href="/auth/forgot-password" className="w-[100%] rounded-[12px] font-semibold text-white h-[52px] flex flex-row items-center justify-center" style={{ backgroundColor: "#641BC4" }}>
                            Request New Link
                        </Link>
                    </div>
                ) : !submitted ? (
                    <>
                        <div className="w-[90%] space-y-5">
                            <div className="flex flex-col items-center">
                                ...
                                <p className="text-center text-sm">Your new password must be different from previously used password</p>
                            </div>
                            ...
                            {formData.map((form) => (
                                <div key={form.name} className="flex flex-col w-[100%]">
                                    ...
                                    {form.name == 'password' && !password && <p className="text-red-500 text-sm">Password must be at least 8 characters</p>}
                                    {form.name == 'confirmPassword' && data.confirmPassword && !confirmPassword && <p className="text-red-500 text-sm">Passwords do not match</p>}
                                </div>
                            ))}
                        ...
                        <div className="w-[100%] flex justify-center">
                            <button
                                ...
                                style={
                                    loading || !password || !confirmPassword ? { backgroundColor: "#a166f0" } : { backgroundColor: "#641BC4" }
                                }
                    ...
                    <div className="w-[90%] space-y-5 flex flex-col items-center">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-sm text-gray-600">Your password has been successfully reset. Click below to log in magically</p>
                        </div>
                ...
                <div className="w-[90%] text-center mt-4 flex flex-col space-y-2">
                    <p>
                        <Link href="/auth/signin" className="font-semibold flex flex-row items-center justify-center text-[#641BC4] hover:underline">
