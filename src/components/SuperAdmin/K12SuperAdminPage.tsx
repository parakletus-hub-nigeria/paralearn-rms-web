...
import { setK12Unlocked, clearK12Unlocked } from "@/reduxToolKit/superAdmin/superAdminSlice";
import {
...
const K12_ADMIN_PASSWORD = "paralearn-k12";
...
function LockScreen() {
  ...
  const handleUnlock = () => {
    if (!pass.trim()) return toast.error("Enter the platform password");
    if (pass.trim() !== K12_ADMIN_PASSWORD)
      return toast.error("Incorrect password");
    dispatch(setK12Unlocked());
...
function ReportCardTemplateManager() {
  const { data: templates = [], isLoading } = useGetSuperAdminTemplatesQuery();
