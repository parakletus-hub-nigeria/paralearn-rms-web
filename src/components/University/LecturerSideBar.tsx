...
export const uniLecturerRoutes = {
  DASHBOARD: "/uni-lecturer/dashboard",
  ATTENDANCE: "/uni-lecturer/attendance",
  ASSESSMENTS: "/uni-lecturer/assessments",
  TIMETABLE: "/uni-lecturer/timetable",
};

const UniLecturerSideBar = ({ children }: { children: ReactNode }) => {
  ...
  const sideBarContent = useMemo(
    () => [
      { label: "Dashboard", path: uniLecturerRoutes.DASHBOARD, icon: Home },
      { label: "Attendance", path: uniLecturerRoutes.ATTENDANCE, icon: MapPin },
      {
        label: "Assessments",
        path: uniLecturerRoutes.ASSESSMENTS,
        icon: ClipboardList,
      },
      {
        label: "Timetable",
        path: uniLecturerRoutes.TIMETABLE,
        icon: CalendarDays,
      },
    ],
    [],
...
const SidebarContentContainer = ({
  ...
  useEffect(() => {
    ...
    if (!sidebar) return;
    const savedScrollPos = sessionStorage.getItem("uni-lec-sidebar-scroll-pos");
    if (savedScrollPos) {
      ...
      <Sidebar className="border-r border-purple-100/50 bg-white">
        <SidebarHeader className="p-4 sm:p-5 pb-2 relative">
          ...
          <div className="flex flex-col items-center text-center gap-1 mt-2">
            <Link href={uniLecturerRoutes.DASHBOARD} className="block shrink-0">
              <Image
                src={logo}
                width={930}
                height={479}
...
export default UniLecturerSideBar;
